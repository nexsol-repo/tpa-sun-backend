...
public class InsuranceController {
    ...
    public ApiResponse<InsuranceResponse> getApplication(
    ...
    /**
     * [Step 1] 신규 가입 시작 (약관 동의)
     * - 무조건 새로운 청약서 생성
     */
    @PostMapping("/start")
    public ApiResponse<InsuranceResponse> start(
            @AuthenticationPrincipal Long userId,
            @RequestBody @Valid InsuranceStartRequest request) {

        InsuranceApplication app = insuranceApplicationService.savePlantInit(
                userId,
    ...
    public ApiResponse<InsuranceResponse> savePlant(
            @PathVariable Long applicationId,
    ...
    public ApiResponse<InsuranceResponse> saveCondition(
            @PathVariable Long applicationId,
            @RequestBody InsuranceConditionRequest request) { // @Valid 없음 (임시저장)

        InsuranceApplication app = insuranceApplicationService.saveCondition(applicationId, request.toInsuranceCondition());
        return ApiResponse.success(InsuranceResponse.of(app));
    }

    /**
     * [Step 4] 최종 가입 완료
     * - 필수 값 검증은 Service 내부 validateForCompletion()에서 수행
     */
    @PostMapping("/{applicationId}/complete")
    public ApiResponse<InsuranceResponse> complete(
            @PathVariable Long applicationId) {
