...
public class UserControllerTest extends RestDocsTest {
    ...
    void me() {
        ...
        webTestClient.get()
                ...
                .isOk()
                .expectBody()
                .consumeWith(document("user-me",
                        requestPreprocessor(),
                        responsePreprocessor(),
                        requestHeaders(
                                headerWithName("Authorization").description("Bearer Access Token (필수)")
                        ),
                        responseFields(
	...
	void update() {
		...
		webTestClient.patch() // 보통 수정은 PATCH or PUT
				...
				.expectStatus().isOk()
				.expectBody()
				.consumeWith(document("user-update",
						requestPreprocessor(),
						responsePreprocessor(),
						requestHeaders(
						...
						requestFields(
								fieldWithPath("applicantName").type(JsonFieldType.STRING).description("변경할 신청자명").optional(),
								fieldWithPath("applicantEmail").type(JsonFieldType.STRING).description("변경할 이메일 (인증 필요)").optional(),
								fieldWithPath("applicantPhoneNumber").type(JsonFieldType.STRING).description("변경할 연락처").optional()
						),
						responseFields(
